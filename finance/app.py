...
def index():
    ...
    transactions_db = db.execute("SELECT symbol, SUM(shares) AS shares, AVG(price) AS price FROM transactions WHERE user_id = ? GROUP BY symbol HAVING SUM(shares) > 0", user_id)
